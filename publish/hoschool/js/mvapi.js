...
(function(window){
	...
	var defaultSettings = {
			...
			tcps: {
				// TCPS 키
				key: 'MTIxLjE3MC4xNjQuMTUyOjYwMDE',
				// 세컨 아이피 사용 여부, v7.1.3
				secondIp: false
			},
			...
			client: {
				...
				mobile: {
					...
					scheme: 'shezviewx',
					// 패키지 이름
					packagename: 'com.saeha.ezViewX',
				},
				// 사용언어 - 없으면 클라이언트 기본 설정에 따라 동작
				language: '',
	...
	window.addEventListener('message', function(e) {
		if(e.data.multiview){
			...
			if(e.data.multiview.event == 'close'){
				jQuery('#multiview_install').remove();
	...
	var ClientDownload = {
			installGuide: function(){
				...
				if(defaultSettings.installPage == 'popup'){
					...
					var $if = jQuery('<iframe id="multiview_install" class="multiview_install" style="z-index: 99999; border: 0; position: fixed; left: 0; top: 0; width: 100%; height: 100%" scrolling="yes"></iframe>');
					$if.attr('src', url);
					
					jQuery('body').append($if).css('overflow', 'hidden');
				}
			},
			exec: function(requestMsg, successCallback, errorCallback){
				...
				if(platformType == 'ANDROID' || platformType == 'IOS' || platformType == 'WINDOWS'){
	...
	var Device = {
			...
			_userAgent: null,
			// 단말 구분
			platformType: {Windows: 'WINDOWS', Mac: 'Macintosh', Android: 'ANDROID', iOS: 'IOS', Unknown: 'UNKNOWN'},
			// 단말 종류
			platform: null,
			...
			_parameterMaker: {
				...
				confOption: function(command, requestMsg){
					...
					if(typeof requestMsg.roomOption == 'object'){
						var ro = [];
						jQuery.each(requestMsg.roomOption, function(key, value){
							ro.push(key + '=' + value);
						});
	...
	var WebAgent = {
			...
			running: function(successCallback, errorCallback){
				var THIS = this;
				// 옵션
				var ao = jQuery.extend({}, this._ajaxOptions);
				// URL
			...
			// 접속 HOST 생성
			_makeHost: function(){
				var protocol = defaultSettings.agent.onlyHttps ? 'https:' : widows.location.protocol;
			...
			_retuenCode: {
				OK: 'OK', 												// 성공
				WA_MISS_VERSION: 'WA_MISS_VERSION', 					// WebAgent
																		// 버전
																		// 불일치
				WA_UNKNOWN_PARAMETER: 'WA_UNKNOWN_PARAMETER',			// WebAgent
																		// 알 수
																		// 없는
																		// 파라메터
																		// 요청
				CLIENT_NO_ANSWER: 'CLIENT_NO_ANSWER', 					// Client
																		// 응답없음
				CLIENT_MISS_VERSION: 'CLIENT_MISS_VERSION',				// Client
																		// 버전
																		// 불일치
				CLIENT_FAIL_ALREADYEXIST: 'CLIENT_FAIL_ALREADYEXIST', 	// Client
																		// 오류
																		// (이미
																		// 입장된Client있음)
				CLIENT_MISS_PASSWORD: 'CLIENT_MISS_PASSWORD', 			// Client
																		// 입장오류
																		// (비밀번호
																		// 오류)
				CLIENT_FAIL: 'CLIENT_FAIL', 							// Client
																		// 오류
																		// (그외.
																		// Unknown)
				OK_BANISHFORCE: 'OK_BANISHFORCE', 						// 멀티룸
																		// 강제퇴장
				OK_CLOSURE: 'OK_CLOSURE', 								// 멀티룸
																		// 종료
				
				CLIENT_RUNNING: 'CLIENT_RUNNING',						// 클라이언트
																		// 실행 중
				CLIENT_NOT_RUNNING: 'CLIENT_NOT_RUNNING',				// 클라이언트
																		// 실행 중
																		// 아님
			}
	}
			...
			_exec_android: function(scheme, successCallback, errorCallback){
				...
				if(defaultSettings.client.mobile.store){
					...
					if(Device.ext.chromeVersion >= 42){
						...
						var $checkframe = jQuery('<iframe></iframe>', {
...
// return false;
	...
	var MvApi = {
			...
			defaultSettings: function(settings){
				...
				jQuery.extend(true, defaultSettings, settings);
