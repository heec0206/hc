...
/*		              KEDI2019 공통함수                         */	
/* ############################################################ */
var searchOfcdcUrl = "/mngs/common/selectOfcdcNmList.do";
var searchSchulUrl = "/mngs/common/selectSchulNmList.do";

/**
 * 검색 목록 조회(셀렉트박스 자동완성)
 * @param falg 		검색 구분
 * @param selBoxId	변경될 셀렉트박스 아이디
 * @param pData		파라미터 데이터(형식 : {key:value, key:value, ...} )
 * 
 * 검색구분코드
 * ( ofcdc : 교육청, schul : 학교)
 * 
 */
function jsSetSearchList(flag, selBoxId, pData){
	var url = "";
	if(flag == "ofcdc") {
		url = searchOfcdcUrl;
	} else if(flag == "schul") {
	...
	var val = jQuery("#"+selBoxId).val();
	selectbox_deletelist(selBoxId);
	jQuery.ajax({
		url : url,
		cache : false,
		async : false,
		...
		data : pData,
		success : function (data){
			selectbox_insertlist(selBoxId, data.list);
			jQuery("#"+selBoxId).val(val);
		}
	});
}

function jsSetSearchText(selBoxId, chgBoxId){
	var val =jQuery("#"+selBoxId).val();
	...
	jQuery("#"+chgBoxId).val(jQuery("#"+selBoxId+" option:selected").text());
	jQuery("#"+selBoxId).val(val);
}

/**
 * 교육청 검색(input, selectBox 같이 있는경우)
 * @param selBoxId 셀렉트박스 ID명
 * @param inputId  인풋 ID명
 */
function fn_setOfcdcNmSelBox(selBoxId, inputId){
	jsSetSearchList("ofcdc", selBoxId, "");
	
	jQuery("#"+inputId).blur(function(){
		jsSetSearchList("ofcdc", selBoxId, {p_search_txt: jQuery("#"+inputId).val()});
	});
	
	//교육청 SELECTBOX선택시 TEXTBOX 변경 이벤트
	jQuery("#"+selBoxId).change(function(){
		jsSetSearchText(selBoxId, inputId);
		jsSetSearchList("ofcdc", selBoxId, {p_search_txt: jQuery("#"+inputId).val()});
	});
}
/**
 * 학교 검색(input, selectBox 같이 있는경우)
 * @param selBoxId 셀렉트박스 ID명
 * @param inputId  인풋 ID명
 */
function fn_setSchlNmSelBox(selBoxId, inputId){
	jsSetSearchList("schul", selBoxId, {});
	
	jQuery("#"+inputId).blur(function(){
		jsSetSearchList("schul", selBoxId, {p_search_txt: jQuery("#"+inputId).val()});
	});
	
	//학교 SELECTBOX선택시 TEXTBOX 변경 이벤트
	jQuery("#"+selBoxId).change(function(){
		jsSetSearchText(selBoxId, inputId);
		jsSetSearchList("schul", selBoxId, {p_search_txt: jQuery("#"+inputId).val()});
	});
