...
function Tree(tid, rootCode, rootName)
{
   ...
   this.leftSideForIE = function(node)
   {
      ...
      renderString = renderString + "<SPAN style='color:" + fg + ";background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'><font color='white'>" + node.name + "</a></SPAN>";
      return renderString;
   }
   /**
    * generate leftside
...
function TreeNode(chasiid, code, upcode, name, filename, type, completeyn, arrnum, contsid)
{
   /**
    * node index
    */
