...
jQuery(function(){
	jQuery.ajaxSetup({
		beforeSend : function(xmlHttpRequest){
			//2019.11.05 화상솔루션에서 AJAX호출시 오류로 추가.
			if(typeof MvApi !== "undefined" && MvApi.ISAJAX != true ){
...
//		}
	...
	/**
	 * 학습자 사이트 비밀번호 체크(특수문자포함)
	 * @param p_pwd		문자열
	 * @param obj		구분문자
	 * @returns
	 */
	function checkPasswordKedi(p_pwd, obj) {
		var chk_pwd = p_pwd;
		var sp_filter =  /[~!@\#$%<>^&*\()\-=+_\']/gi;
		var blank_filter = /[\s]/g;
		...
		if(chk_pwd.length < 9  || chk_pwd.length > 13){
			alert("비밀번호는 영문, 숫자, 특수문자를 포함하여 9자리 이상 13자리 이하이어야 합니다.");
			obj.focus();
			return false;
		}
		
		if(chk_num <0 || chk_eng <0 || chk_sp < 0){
			alert("비밀번호는 영문, 숫자, 특수문자를 혼용하여야 합니다.");
			obj.focus();
			return false;
		}
		if(/(\w)\1\1\1/.test(chk_pwd)){
			alert("비밀번호에 같은 문자를 4번 이상 사용하실수 없습니다.");
			obj.focus();
			return false;
		}
		if(blank_filter.test(chk_pwd)){
	...
	**/
	 ...
	 * @returns
	 */
	function fn_dt_datepicker_call(date_calendar_yn, datefmt,clearYn, maxDate, minDate, yearRange, hidden, view, event){
		var dateFormat = "";
		if(datefmt == "YM") {
			dateFormat = "yy-mm";
		} else {
		...
		jQuery(".dateSet").each(function(){
			...
			jQuery(this).datepicker({
				...
				yearRange : yearRange === 'undefined' ? "c-10:c+10" : yearRange,
				dayNamesMin: jslang  =="ko" ? [  "일", "월", "화", "수", "목", "금", "토" ] : [ "Su","Mo","Tu","We","Th","Fr","Sa"], 
				monthNamesShort : jslang  =="ko" ? [ '1월','2월','3월','4월','5월','6월','7월','8월','9월','10월','11월','12월' ] : [ "Jan", "Feb", "Mar", "Apr", "Maj", "Jun","Jul", "Aug", "Sep", "Okt", "Nov", "Dec" ],
				beforeShow: function() {
					if(date_calendar_yn == "Y") {
